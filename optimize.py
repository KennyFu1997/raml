...
def objective(args, datasets, t_model, trial, device=None):
    ...
    mAP, _ = run_train(args, datasets, s_model, t_model, logger, trial=trial)
    return mAP
...
def optimize(args, cfgs, t_model, feats, logger, device=None):
    ...
    study.optimize(lambda trial: objective(args, train_split, t_model, trial, device=device), n_trials=TRIALS, callbacks=[test_callback])
    ...
    print(f"updated lr: {args.lr}")
    print(f"updated lam_1: {args.lam_1}")
    print(f"updated lam_2: {args.lam_2}")
