...
def sym_train(input, s_model, criterion, train_loader, ps=None):
    ...
    pos = mine_hardest(scores, input[1].to(s_model.device), input[1].to(s_model.device), ni-1, negative=False)
    neg = s_model(torch.flatten(input[2].to(s_model.device), end_dim=1).to(s_model.device, non_blocking=True), ps=ps)
    neg = neg.t().reshape(input[1].shape[0], nn, -1)
...
def train_epoch(train_loader, s_model, criterion, train_fn, optimizer, epoch, scheduler, update_every):
    ...
    for i, input in enumerate(train_loader):
        #print(f"input[0] device: {input[0].device}")
        #print(f"s_model device: {s_model.device}")
        ps = np.random.choice([48, 40, 30, 24, 20, 15, 12, 10, 8])
        #print(f"ps: {ps}")
        loss = train_fn(input, s_model, criterion, train_loader, ps=ps)
...
def run_train(args, datasets, s_model, t_model, logger, trial=None):
    ...
    if trial is None:
        save_checkpoint(s_model, optimizer, scheduler, val_map, epoch, args.directory)
    np.save(f'losses{str(args.image_size)}.npy', np.array(losses))
    np.save(f'maps{str(args.image_size)}.npy', np.array(maps))
