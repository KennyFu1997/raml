...
def run_tests(datasets, t_model, s_model, image_size, t_image_size, logger=None, asym=True, sym=True, ms=(1,), msp=1):
    ...
    for dataset in datasets:
        with torch.no_grad():
            asym_mAP, sym_mAP = test(t_model, s_model, dataset, image_size, t_image_size, ms, msp, logger, asym=asym, sym=sym)
            results_asym.append(asym_mAP)
            results_sym.append(sym_mAP)

    if asym:
        avg_score_asym = np.mean(results_asym)
        return avg_score_asym
    else:
        avg_score_sym = np.mean(results_sym)
        return avg_score_sym
...
def test(net_teacher, net_student, dataset, image_size, t_image_size, ms, msp, logger=None, asym=True, sym=True):
    ...
    ps = 40
    qvecs = dataset.extract_query(net_student, ms, msp, image_size, ps=ps)

    print('>> {}: Evaluating...'.format(dataset.name))
    asym_mAP = None
    if asym:
        vecs = dataset.load_or_extract_db(net_teacher, ms, msp, t_image_size, 15)
