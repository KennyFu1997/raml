...
def regression_loss(x1, x2, eps=1e-6):
    dif = 1 - (x1 * x2).sum(-1) # Dist^2 / 2
    D = torch.pow(dif + eps, 2) # (D^2) / 4
    return torch.sum(D.mean(-1)) # 逐渐接近0
...
def chamfer_sim(x1, x2, chunk_size=300):
    scores = torch.zeros((x1.shape[0], x2.shape[0], x1.shape[1], x2.shape[1]))
    for k in range(x2.shape[0] // chunk_size + 1):
        scores[:, k * chunk_size:(k + 1) * chunk_size] = torch.einsum('kji,mli->kmjl',
                                                      x1, x2[k * chunk_size:(k + 1) * chunk_size])
    scores = (scores.amax(dim=-2) + scores.amax(dim=-1)).mean(dim=-1)
    return scores