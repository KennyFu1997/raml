...
scheduler_names = ['onecycle', 'cyclic', 'exponential', 'cosine', 'no_scheduler']
...
def create_parser():
    ...
    parser.add_argument('--mode', '-m', metavar='MODE', default='ts_aug',
                        ...
                        help='training mode options: ' +
                            ' | '.join(mode_names) +
                            ' (default: ts_aug)')
    parser.add_argument('--loss-margin', '-lm', metavar='LM', default=0.3, type=float,
                        help='loss margin: (default: 0.3)')
    ...
    # new args
    parser.add_argument('--pretrained-source', choices=['i1k', 'i21k'],
                    help='pretrained model source (choices: i1k, i21k)')
    parser.add_argument('--ep', type=int, choices=[300, 600, 1000, 1200],
                        help='number of epochs (choices: 300, 600, 1000, 1200)')
    parser.add_argument('--version', choices=['flexivit', 'vit'],
                        help='model version (choices: flexivit, vit)')
