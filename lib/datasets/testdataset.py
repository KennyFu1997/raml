...
class FineGrainTestset(TestDataset):
    ...
    def extract_db(self, model, ms, msp, imsize, ps=None):
        # testset一开始不保存transform，在extract的时候在用transofrm
        return extract_vectors(model, self.images, None, fg_eval_transform(imsize, mean=model.meta['mean'], std=model.meta['std']),
                               ms=ms, msp=msp, workers=self.num_workers, batch_size=self.batch_size, ps=ps)

    def extract_query(self, model, ms, msp, imsize, ps=None):
        return self.extract_db(model, ms, msp, imsize, ps=ps)

    def extract_all(self, model, ms, msp, imsize):
        return self.extract_db(model, ms, msp, imsize)

    def load_or_extract_db(self, *args):
        return self.extract_db(*args)
