...
from lib.datasets.traindataset import RetrievalDataset, mine_dist, mine_dist_v2
...
class TuplesFineGrainDataset(FineGrainDataset):
    ...
    def create_epoch_tuples(self, net):
        ...
        ## ------------------------
        ## ps for each batch (tuples)
        ## ------------------------
        #values = [48, 40, 30, 24, 20, 15, 12, 10, 8]
        #self.ps_pool = np.random.choice(values, size=4000)
...
class FineGrainRegressionTS(FineGrainDataset):
    def __init__(self, *args, teacher_feat, **kwargs):
        ...
        self.transform = fg_eval_transform(self.imsize, mean=self.mean, std=self.std)
