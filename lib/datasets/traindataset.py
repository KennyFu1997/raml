...
def mine_dist_v2(vecs, qvecs, cls, qcls, nnum):
    dim = qvecs.shape[0]
    prod = torch.mm(vecs.t(), qvecs)
    distances = -(2 - 2 * prod).clamp(min=0).sqrt()
    sel_d = distances.shape[0]

    nidxs = []
    start_time = time.time()
    for q in range(qvecs.shape[1]):
        prob = torch.exp(distances[:, q] - torch.max(distances[:, q]))
        prob[np.where(cls == qcls[q])[0]] = 0
        prob += 1e-6
        prob = prob / prob.sum()
        # mixed mining
        if np.random.choice([0, 1]):
            nidxs.append(np.random.choice(sel_d, size=nnum, p=prob.detach().cpu().numpy(), replace=False))
        else:
            nidxs.append(np.random.choice(sel_d, size=nnum, replace=False))
    end_time = time.time()
    print(f"elapsed_time for mining: {end_time - start_time}")
...
class RetrievalDataset(data.Dataset):
    ...
    def get_vecs(self, net, idxs):
        ...
        vecs = torch.zeros(net.meta['outputdim'], len(idxs)).to(net.device)

        ps = np.random.choice([40, 30, 24, 20, 15, 12, 10])
        #ps = 15
        for i, input in enumerate(self.dataloader):
            x = net(input.to(net.device), ps=ps)
