...
def load_features(datasets, args, cfgs, ms=(1,), msp=1, device=None, ps=None):
    feats = {}
    model = None
    ...
    for dataset in datasets:
        ...
        if os.path.exists(feat_path):
            ...
            if model is None:
                model = load_model(args.data_root, args, args.teacher, args.teacher_path, device=device)
                #model.distill_flag = True
                #model.device = device
            vecs = extract_features(dataset, cfgs[dataset], args.teacher_image_size, model, ms, msp, ps=ps)
        feats[dataset] = vecs

    return feats, model

def extract_features(dataset, cfg, image_size, net, ms, msp, ps=None):
    start = time.time()
    ...
    # t 提取 feats用的 transform是对的(且无随机性)
    vecs = extract_vectors(net, list(it.chain(*cfg[mode])), None, transform, ms=ms, msp=msp, batch_size=50, ps=ps)
